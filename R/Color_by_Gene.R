# Color by Gene [scRNA-Seq] [CCBR] (82c9ec27-5cd8-4fd2-87a9-2a0f83e2ac3a): v95

#' @title Visualize gene expression for provided Genes across your cells as a heatmap. 
#' @description You should see one plot (TSNE or UMAP, your choice) per gene name provided. The intensity of the red color will be relative to the expression of the gene in that cell.
#' @details This function must be run downstream of the Sample Names function, as well as be provided a combined Seurat Object such as the one produced by the SingleR Cell Annotation function
#' 
...
#' @param return.seurat.object Set to FALSE if you want only a geneset (and not a Seurat object) to be returned.
#' @param color The color you want to use for the "heat" in your heatmap (default "red").
#' @param point.size The size of the points representing each cell in your visualization. Default is 1.
#' @param point.shape The code for your point shape (R "pch" argument). Default is 16.
...
colorByGene <- function(object,
  ...
  nogene = gene[!gene %in% rownames(object.sub[[assay]]@scale.data)]
  ...
  if(length(gene)>0){
    plotgene <- function(gene){
      ...
      reduction_type_x <- paste0(reduction.type, "-1")
      reduction_type_y <- paste0(reduction.type, "-2")
      ...
      g <- ggplot(clusmat, aes(x=umap1, y=umap2)) +
        ...
        ggtitle(gene) +
        geom_point(aes(colour=gene),alpha=point.transparency,shape=point.shape, size=point.size) +
        theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
              panel.background = element_blank(),legend.text=element_text(size=rel(0.5)) )+
        scale_color_gradient(limits = c(0, gene.quant[3]),low = "lightgrey", high = color) +
        xlab(reduction_type_x) + ylab(reduction_type_y)  
      return(g)
    }
