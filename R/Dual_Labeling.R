...
#' @param marker_1_type Slot to use for first marker (choices are "SCT","protein","HTO")
#' @param marker2 Second gene/marker for coexpression analysis
#' @param marker_2_type Slot to use for second marker (choices are "SCT","protein","HTO")
#' @param data_reduction Dimension Reduction method to use for image (options are "umap" or "tsne")
#' @param point_size Point size for image (default is 0.5)
#' @param point_shape Point shape for image (default is 16)
#' @param point_transparency Point transparency for image (default is 0.5)
#' @param add_marker_thresholds Add marker thresholds (default is FALSE)
#' @param marker_1_threshold Threshold set for first marker (default is 0.5)
#' @param marker_2_threshold Threshold set for first marker (default is 0.5)
#' @param filter_data Add new parameter to metadata using marker thresholds (default is FALSE)
#' @param M1_filter_direction Filter to samples that have greater or less than marker 1 threshold (default is "greater than")
#' @param M2_filter_direction Filter to samples that have greater or less than marker 2 threshold (default is "greater than")
#' @param apply_filter_1 If TRUE, apply the first filter (default is TRUE)
#' @param apply_filter_2 If TRUE, apply the second filter (default is TRUE)
#' @param filter_condition If TRUE, apply both filters 1 and 2 and take intersection. If FALSE, apply filters and take the union.
#' @param parameter_name Name for metadata column for new marker filters. (Default is "Marker")
#' @param trim_marker_1 Trim top and bottom percentile of marker 1 signal to pre-scale trim values (below) to remove extremely low and high values (Default is TRUE)
#' @param trim_marker_2 Trim top and bottom percentile of marker 2 signal to pre-scale trim values (below) to remove extremely low and high values (Default is TRUE)
#' @param pre_scale_trim Set trimming percentile values (Defalut is 0.99)
#' @param density_heatmap Creates a additional heatmap showing the density distribution of cells. (Default is FALSE)
#' @param display_unscaled_values Set to TRUE if you want to view the unscaled gene/protein expression values (Default is FALSE)
...
gg.overlay <- function(so.sub,df,marker1,marker2){
  ...
  gg.z1 <- ggplot(df, aes(dr1,dr2))+
    geom_point(color=rgb(red=df$mark1.scale,green=0,blue=0),shape=point_shape,size=point_size, alpha=point_transparency)+
    theme_classic() +
    xlab(paste0(data_reduction,"-1")) + 
    ylab(paste0(data_reduction,"-2")) +
    ggtitle(marker1) +
    coord_fixed()
  ...
  gg.z2 <- ggplot(df, aes(dr1,dr2))+
    geom_point(color=rgb(red=0,green=df$mark2.scale,blue=0),shape=point_shape,size=point_size, alpha=point_transparency)+
    theme_classic() +
    xlab(paste0(data_reduction,"-1")) + 
    ylab(paste0(data_reduction,"-2")) +
    ggtitle(marker2) +
  ...
  gg <- ggplot(df, aes(dr1,dr2))+
    geom_point(color=rgb(red=df$mark1.scale,green=df$mark2.scale,blue=0),shape=point_shape,size=point_size, alpha=point_transparency)+
    theme_classic() +
    xlab(paste0(data_reduction,"-1")) + 
    ylab(paste0(data_reduction,"-2")) +
    ggtitle("Combined") +
...
gg.overlay2 <- function(so.sub,df,marker1,marker2){
  ...
  if(display_unscaled_values == TRUE){
  ...
  gg.z1 <- ggplot(df, aes(mark1.scale,mark2.scale))+
    geom_point(color=rgb(red=df$mark1.scale,green=0,blue=0),shape = 20,size=point_size)+
    theme_classic() +
    xlab(label1) + 
    ylab(label2) +
    coord_fixed()
  ...
  gg.z2 <- ggplot(df, aes(mark1.scale,mark2.scale))+
    geom_point(color=rgb(red=0,green=df$mark2.scale,blue=0),shape = 20,size=point_size)+
    theme_classic() +
    xlab(label1) + 
    ylab(label2) +
    coord_fixed()
  ...
  gg <- ggplot(df, aes(mark1.scale,mark2.scale))+
    geom_point(color=rgb(red=df$mark1.scale,green=df$mark2.scale,blue=0),shape = 20,size=point_size)+
    theme_classic() +
    xlab(label1) + 
    ylab(label2) +
    coord_fixed()
  
  if(add_marker_thresholds==TRUE){
    gg.z1 <- gg.z1 + 
...
t1 = marker_1_threshold
t2 = marker_2_threshold

#Select marker 1 values and scale:
mark1 = so.sub@assays[[marker_1_type]]@scale.data[marker1,]
if(trim_marker_1 == TRUE){
  q1 = quantile(mark1,pre_scale_trim)
  q0 = quantile(mark1,1-pre_scale_trim)
  mark1[mark1<q0]=q0
  mark1[mark1>q1]=q1
}
...
mark2 = so.sub@assays[[marker_2_type]]@scale.data[marker2,]
if(trim_marker_2 == TRUE){
  q1 = quantile(mark2,pre_scale_trim)
  q0 = quantile(mark2,1-pre_scale_trim)
  mark2[mark2<q0]=q0
  mark2[mark2>q1]=q1
}
...
#Draw Plots:
df <- data.frame(cbind(dr1=so.sub@reductions[[data_reduction]]@cell.embeddings[,1],
                       dr2=so.sub@reductions[[data_reduction]]@cell.embeddings[,2],
...
if(density_heatmap == TRUE){
  ...
  df_heatmap <- data.frame(x = x, y = y, 
                           d = densCols(x, y, nbin = 500, colramp = colorRampPalette(rev(rainbow(10, end = 4/6)))))
...
if(filter_data == TRUE){
  ...
  if(M1_filter_direction == "greater than"){
  ...
  if(M2_filter_direction == "greater than"){
  ...
  colnames(so.sub@meta.data) <- sub("x",parameter_name,colnames(so.sub@meta.data))
  
  cat("\n")
  print("Final Breakdown:")
  print(addmargins(table(so.sub.df[[parameter_name]],so.sub.df$sample_name)))
