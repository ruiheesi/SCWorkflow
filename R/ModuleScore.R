  ...
  #' @param sample.names List of samples to subset the data by
  #' @param sample.to.display List of samples to depict on dimension plot, samples not in the list would be colored gray in the background
  #' @param geneset.dataframe Table of marker genes for each celltype (column names of the table), append "_prot" or "_neg" for proteins or negative markers
  #' @param proteins.presence Set to TRUE if there are CITE-seq markers in geneset.dataframe
  #' @param celltypes.to.analyze Vector of celltypes from geneset.dataframe to screen for
  #' @param manual.threshold Specify bimodal thresholds for cell classification, should be of the same length as celltypes.to.analyze vector
  #' @param general.class Base population of cells to classify
  #' @param multi.level.class Toggle to TRUE if there are subpopulations of cells you want to screen for
  #' @param levels.dataframe Table of subpopulation levels (column names of the table) and parent-child information (e.g. Tcells-CD4, Tcells-CD8)
  ...
  #' @param gradient.density.font.size Set size of axis labels on gradient density plot of ModuleScore distribution
  #' @param violinplot.font.size Set size of axis labels on violin plot of ModuleScore distribution
  #' @param step.size Set step size of distribution plots
...
moduleScore <- function(SO, 
                         ...
                         sample.to.display, 
                         geneset.dataframe, 
                         proteins.presence = FALSE, 
                         celltypes.to.analyze,
                         manual.threshold = c(0), 
                         general.class, 
                         multi.level.class = FALSE, 
                         levels.dataframe, 
                         reduction = "tsne",
                         nbins = 24,
                         gradient.density.font.size = 6,
                         violinplot.font.size = 6,
                         step.size = 0.1
                         ){
  ...
  Predict_Cell_from_ModScore <- function(ModScore_Metadata,manual.threshold,rejection){
    
    thres_ls <- list()
    for (i in 1:ncol(ModScore_Metadata)){
      thres_ls[[i]]<- rep(manual.threshold[i],nrow(ModScore_Metadata))
    }
  ...
  geneset_for_module_scores <- unlist(geneset.dataframe)
  ...
  if (length(sample.names) == 0) {
    sample.names = unique(SO@meta.data$sample_name)
  }
  ...
  if("active.ident" %in% slotNames(SO)){
    ...
    SO.sub = subset(SO, ident = sample.names)
  } else {
    sample_name = as.factor(SO@meta.data$orig.ident)
    names(sample_name)=names(SO@active.ident)
    SO@active.ident <- as.factor(vector())
  ...
  marker = select(geneset.dataframe, celltypes.to.analyze)
  ...
  if (length(manual.threshold) != length(celltypes.to.analyze)){
    if (sum(manual.threshold) == 0){
      manual.threshold <- rep(0, length(celltypes.to.analyze))
  ...
  names(manual.threshold) <- celltypes.to.analyze
  ...
  for (i in seq_along(marker.list)) { 
    ...
    clusmat <- mutate(clusmat, sample_clusid = clusmat$clusid * grepl(paste(sample.to.display, collapse = "|"), clusmat$ident))
    ...
    g <- ggplot(clusmat, aes(x=umap1, y=umap2)) +
      ...
      scale_color_gradientn(colours = c("blue4","lightgrey", "red"), values = scales::rescale(c(0,manual.threshold[i]/2,manual.threshold[i],(manual.threshold[i]+1)/2,1), limits = c(0, 1))) + guides(colour = guide_legend(override.aes = list(size=5, alpha = 1))) +
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
      xlab("tsne-1") + ylab("tsne-2")
    
    g1 = RidgePlot(SO.sub,features=m,group.by="orig.ident") + theme(legend.position = "none", title = element_blank(), axis.text.x = element_text(size = gradient.density.font.size)) + geom_vline(xintercept = manual.threshold[i], linetype = "dashed", color = "red3") + scale_x_continuous(breaks = seq(0,1,step.size))
    
    # Violin Plot
    g2 = ggplot(clusid.df,aes(x=id,y=ModuleScore)) + geom_violin(aes(fill=id)) +  theme_classic() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.title = element_blank(), panel.background = element_blank(), axis.text.x=element_blank(),legend.text=element_text(size=rel(0.8)),legend.position="top", axis.text.y = element_text(size = violinplot.font.size)) + guides(colour = guide_legend(override.aes = list(size=5, alpha = 1))) + geom_hline(yintercept = manual.threshold[i], linetype = "dashed", color = "red3") + scale_y_continuous(breaks = seq(0,1,step.size))
    ...
    g3 = ggplot(data.frame(x = d$x, y = d$y), aes(x, y)) + xlab("ModuleScore") + ylab("Density") + geom_line() + 
      ...
      scale_color_gradientn(colours = c("blue4","lightgrey", "red"), values = scales::rescale(c(0,manual.threshold[i]/2,manual.threshold[i],(manual.threshold[i]+1)/2,1), limits = c(0, 1))) + geom_vline(xintercept = manual.threshold[i], linetype = "dashed", color = "red3") + geom_vline(xintercept = manual.threshold[i], linetype = "dashed", color = "red3") + scale_x_continuous(breaks = seq(0,1,step.size)) + theme(legend.title = element_blank(), axis.text.x = element_text(size = 6))
  ...
  general.class <- general.class[general.class %in% colnames(SO.sub@meta.data)]
  
  # Subset the columns of the metadata containing module scores only
  SO_Trunc_Metadata_General <- SO.sub@meta.data[general.class]
  
  General_thres_vec <- manual.threshold[general.class]
  ...
  if (multi.level.class){   
    
    for (k in 1:ncol(levels.dataframe)){ 
      ...
      Sub_class_storage <- levels.dataframe[[k]][!is.na(levels.dataframe[[k]])]
      ...
      for (parent in parent_class){
        ...
        for (children in children_class){
          ...
          adjusted_density_plot <- ggplot(SO_Trunc_Metadata_parents, aes_string(x = children)) + geom_density() + ggtitle(plot_title) +                  geom_vline(xintercept = manual.threshold[children], linetype = "dashed", color = "red3")
        ...
        children_thres_vec <- manual.threshold[children_class]
